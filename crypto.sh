...
#
# You can start, stop, and query the bot's status: ./crypto.sh [start|stop|status]
#
lib_dir=./libs
...
crypto_jar=crypto-app-1.0.1.jar
...
case "$1" in
   'start')
       if [[ -e ${pid_file} ]]; then
          ...
          java -Xmx64m -Xss256k -Dlog4j.configurationFile=file:${log4j2_config} --illegal-access=deny -jar ${lib_dir}/${crypto_jar} 2>&1 >/dev/null &
